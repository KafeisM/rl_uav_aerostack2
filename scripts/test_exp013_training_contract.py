#!/usr/bin/env python3
"""ROS-free checks for the Exp013 predictive-guard training contract.

Exp013 keeps the exp012 tutor-spec reward, safe-area sampling and retry
...
Usage:
    conda run -n rl_uav python scripts/test_exp013_training_contract.py
"""
...
EXP013_CONFIG = REPO_ROOT / 'configs' / 'train_ppo_phase1_exp013.yaml'
...
def load_exp013_config() -> dict:
    with EXP013_CONFIG.open('r', encoding='utf-8') as handle:
        return yaml.safe_load(handle)


def make_exp013_env(**overrides) -> SamplerTestEnv:
    kwargs = build_env_kwargs(load_exp013_config()['environment'])
...
def test_config_declares_predictive_guard_and_env_constructs() -> None:
    env_cfg = load_exp013_config()['environment']
    ...
    assert env_cfg['bounds_guard_lookahead_s'] == 0.6
    assert env_cfg['bounds_guard_vertical_push_speed'] == 0.5
    assert env_cfg['low_altitude_guard_lookahead_s'] == 0.6
    ...
    env = make_exp013_env()  # must not raise ValueError
    assert env.bounds_action_guard is True
    assert env.bounds_guard_lookahead_s == 0.6
    assert env.bounds_guard_vertical_push_speed == 0.5
    assert env.low_altitude_guard_lookahead_s == 0.6


def test_built_env_kwargs_carry_predictive_parameters() -> None:
    kwargs = build_env_kwargs(load_exp013_config()['environment'])
    assert kwargs['bounds_guard_lookahead_s'] == 0.6
    assert kwargs['bounds_guard_vertical_push_speed'] == 0.5
    assert kwargs['low_altitude_guard_lookahead_s'] == 0.6
...
def test_guard_lines_strictly_enclose_sampling_volume() -> None:
    ...
    env = make_exp013_env(step_duration=load_exp013_config()['environment']['step_duration'])
...
def test_guard_activity_is_monitored() -> None:
    keywords = load_exp013_config()['environment']['monitor_info_keywords']
...
def test_reward_and_ppo_hyperparameters_match_exp012_tutor_spec() -> None:
    ...
    assert config['experiment']['name'] == 'ppo_phase1_exp013_predictive_guard_50k'
    assert config['training']['checkpoint_prefix'] == 'ppo_phase1_exp013'
...
def main() -> int:
    ...
    print('✓ PASS: Exp013 predictive-guard training contract holds')
    return 0

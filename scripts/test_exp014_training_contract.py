#!/usr/bin/env python3
"""ROS-free checks for the Exp014 predictive-guard training contract.

Exp014 keeps the exp012 tutor-spec reward, safe-area sampling and retry
...
Usage:
    conda run -n rl_uav python scripts/test_exp014_training_contract.py
"""
...
EXP014_CONFIG = REPO_ROOT / 'configs' / 'train_ppo_phase1_exp014.yaml'
...
def load_exp014_config() -> dict:
    with EXP014_CONFIG.open('r', encoding='utf-8') as handle:
        return yaml.safe_load(handle)


def make_exp014_env(**overrides) -> SamplerTestEnv:
    kwargs = build_env_kwargs(load_exp014_config()['environment'])
...
def test_config_declares_predictive_guard_and_env_constructs() -> None:
    env_cfg = load_exp014_config()['environment']
    ...
    assert env_cfg['max_start_target_distance'] == 2.5
    assert env_cfg['min_start_target_distance'] == 1.0

    env = make_exp014_env()  # must not raise ValueError
...
def test_guard_lines_strictly_enclose_sampling_volume() -> None:
    ...
    env = make_exp014_env(step_duration=load_exp014_config()['environment']['step_duration'])
...
def test_guard_activity_is_monitored() -> None:
    keywords = load_exp014_config()['environment']['monitor_info_keywords']
...
def test_reward_and_ppo_hyperparameters_match_exp012_tutor_spec() -> None:
    ...
    assert config['experiment']['name'] == 'ppo_phase1_exp014_curriculum_close_50k'
    assert config['training']['checkpoint_prefix'] == 'ppo_phase1_exp014'
...
def main() -> int:
    ...
    print('✓ PASS: Exp014 curriculum training contract holds')
    return 0

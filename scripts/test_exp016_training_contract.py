#!/usr/bin/env python3
"""ROS-free checks for the Exp016 predictive-guard training contract.

Exp016 keeps the exp012 tutor-spec reward, safe-area sampling and retry
...
Usage:
    conda run -n rl_uav python scripts/test_exp016_training_contract.py
"""
...
EXP016_CONFIG = REPO_ROOT / 'configs' / 'train_ppo_phase1_exp016.yaml'
...
def load_exp016_config() -> dict:
    with EXP016_CONFIG.open('r', encoding='utf-8') as handle:
        return yaml.safe_load(handle)


def make_exp016_env(**overrides) -> SamplerTestEnv:
    kwargs = build_env_kwargs(load_exp016_config()['environment'])
...
def test_config_declares_predictive_guard_and_env_constructs() -> None:
    env_cfg = load_exp016_config()['environment']
    ...
    env = make_exp016_env()  # must not raise ValueError
...
def test_guard_lines_strictly_enclose_sampling_volume() -> None:
    ...
    env = make_exp016_env(step_duration=load_exp016_config()['environment']['step_duration'])
...
def test_guard_activity_is_monitored() -> None:
    keywords = load_exp016_config()['environment']['monitor_info_keywords']
...
def test_reward_and_ppo_hyperparameters_match_exp012_tutor_spec() -> None:
    ...
    assert config['experiment']['name'] == 'ppo_phase1_exp016_curriculum_full_150k'
    assert config['training']['checkpoint_prefix'] == 'ppo_phase1_exp016'
...
def main() -> int:
    ...
    print('✓ PASS: Exp016 curriculum training contract holds')
    return 0
